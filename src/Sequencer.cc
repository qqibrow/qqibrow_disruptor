...
Sequencer::Sequencer(int bufferSize, WaitStrategy* strategy) :
    ...
    waitStrategy_(strategy)
{
    assert(bufferSize > 1 && ((bufferSize & (bufferSize - 1)) == 0) && "buffSize is wrong.");
}

long Sequencer::GetCursor() const {
    return cursor_.Get();
}

int Sequencer::GetBufferSize() const {
    return bufferSize_;
}
...
void Sequencer::RemoveGatingSequence(Sequence* sequence) {
    remove_if(gatingSequences_.begin(), gatingSequences_.end(),
            [&sequence](const SequencePtr& ptr) { return ptr.get() == sequence;});
}
long Sequencer::GetMinimumSequence() const {
    return 0;
}
